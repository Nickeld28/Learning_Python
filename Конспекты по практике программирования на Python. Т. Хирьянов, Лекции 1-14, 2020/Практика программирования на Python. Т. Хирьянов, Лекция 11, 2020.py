...
Есть раунд игры, игроки ставят значки на поле по очереди.
Есть окно, на котором будет игровое поле

Есть игрок1 и игрок2, крестик и нолик.
Игра заканчивается когда игрок поставил три своих значка в ряд или не осталось клеток для хода.
Что будет отвечать за определение конца игры.
...
class Cell(Enum):  # создали класс на основе класса Enum. Используются целые числа
    """
    Класс, описывающий клетку игрового поля
    """
    VOID = 0  # для пустой клетки
    CROSS = 1  # для клетки с крестиком
    ZERO = 2  # ждя клетки с ноликом
...
class GameRoundManager:
    ...
    def __init__(self, player1: Player, player2: Player):
        self._players = [player1, player2]  # нижнее подчеркивание в начале означает что это свойство не публичное
...
class GameWindow:
    """
    Содержит в себе виджет поля, а также менеджера игрового раунда
    """
