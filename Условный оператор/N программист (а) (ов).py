...
это значит, что вы не рассмотрели какой-то из случаев входных данных (число программистов 10000≤n≤1000).
...
def take_word_end(num):
    """Данная функция возвращает окончание для слова "программист",
    в зависимости от введенного числа num in range(0, 10)"""
    if num == 1:
        return ''
    elif num in [2, 3, 4]:
        return 'а'
...
n = int(input())
if 1000 >= n >= 0:
    if n not in [11, 12, 13, 14] and n % 100 not in [11, 12, 13, 14]:
        word_end = take_word_end(n % 10)
    else:
        word_end = 'ов'
    print(n, ' программист', word_end, sep='')
else:
